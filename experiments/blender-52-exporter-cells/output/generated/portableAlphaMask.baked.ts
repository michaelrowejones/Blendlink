/* Generated once by blendlink — this file is YOURS to edit and will not
 ...
 */
// blendlink-baked-recipe-template-version: 11
import * as THREE from 'three'
import { portableAlphaMask } from './portableAlphaMask.gen'
...
export function createBakedScene(
  ...
  const states = (portableAlphaMask.states ?? {}) as Record<string, string | GroupedUrl>
  const defaultState = portableAlphaMask.defaultState as string | null
  const bakeOutputs = (portableAlphaMask.bakeOutputs ?? {}) as BakeOutputs
  const stateScales = (portableAlphaMask.stateScales ?? {}) as StateScales
  const lightGroups = (portableAlphaMask.lightGroups ?? {}) as Record<
    string,
    { url: string; maxValue: number } | GroupedLayer
  >
  const visibility = (portableAlphaMask.stateVisibility ?? {}) as StateVisibility
  const textureVariants = ((portableAlphaMask as typeof portableAlphaMask & {
