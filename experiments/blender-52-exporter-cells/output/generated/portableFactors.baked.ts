...
import { portableFactors } from './portableFactors.gen'
...
export function createBakedScene(
  ...
  const states = (portableFactors.states ?? {}) as Record<string, string | GroupedUrl>
  const defaultState = portableFactors.defaultState as string | null
  const bakeOutputs = (portableFactors.bakeOutputs ?? {}) as BakeOutputs
  const stateScales = (portableFactors.stateScales ?? {}) as StateScales
  const lightGroups = (portableFactors.lightGroups ?? {}) as Record<
    string,
    { url: string; maxValue: number } | GroupedLayer
  >
  const visibility = (portableFactors.stateVisibility ?? {}) as StateVisibility
  const textureVariants = ((portableFactors as typeof portableFactors & {
