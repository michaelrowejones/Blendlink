...
class BLENDLINK_UL_atlases(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_prop):
        ...
        output = props.bake_output_ui(item.bake_output)["short"]
...
class BLENDLINK_PT_reflection_probes(_BlendlinkScenePanelMixin, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if status.image is not None:
            # template_preview accepts materials, textures, lights, worlds and
            # line styles - not Images. It rejected this one silently through
            # RNA_warning (which prints and returns rather than raising), so
            # the try/except below it never fired, no picture ever appeared,
            # and every redraw of this panel added a line to the console.
            # template_icon over the image's own preview is the idiom the rest
            # of the add-on uses; the preview itself is generated by the shared
            # timer, never here.
            icon_id = int(getattr(getattr(status.image, "preview", None), "icon_id", 0) or 0)
            if icon_id:
                status_box.template_icon(icon_value=icon_id, scale=6.0)
...
def _draw_atlas_recipe(layout, project, context=None):
    ...
    consequence = layout.box()
    output_ui = props.bake_output_ui(atlas.bake_output)
    consequence.label(text=output_ui["headline"], icon=output_ui["icon"])
    _draw_wrapped(consequence, output_ui["consequence"])
    if index == 0:
...
def _draw_lighting_states_recipe(layout, project, context=None):
    context = context or bpy.context
    _draw_wrapped(
        layout,
...
class BLENDLINK_PT_lighting_states(_BlendlinkScenePanelMixin, bpy.types.Panel):
    ...
    def draw(self, context):
        _draw_lighting_states_recipe(
            self.layout, context.scene.blendlink_project, context,
        )
...
def _effective_rendering(obj, project=None) -> tuple[str, str]:
    ...
    if _authored_rendering_mode(obj) == "BAKED":
        ...
        if atlas is not None:
            return (
                "BAKED",
                f"Explicit Baked choice; {atlas.name} uses "
                f"{props.bake_output_ui(atlas.bake_output)['phrase']}.",
            )
...
def _sidebar_panel_classes() -> tuple:
    """Every panel that lives in the sidebar tab, parents before children.

    Sidebar panels are not all declared in this module: Effects & Behaviors is
    a child of the workspace panel and lives with the rest of the component
    UI. Listing only this module's panels meant renaming the tab unregistered
    and rebuilt the parent while that child stayed bound to the destroyed
    parent type, so the one preference the add-on offers silently deleted a
    panel from the sidebar - on every launch afterwards, because register()
    applies a non-default category at startup too.
    """
    from . import components_ui

    return (*_sidebar_panels, *components_ui.sidebar_panels())


def re_register_category(category: str):
    """Move the viewport workspace without disturbing native Properties panels."""
    panels = _sidebar_panel_classes()
    for cls in reversed(panels):
        try:
            ...
            pass
    for cls in panels:
        # Only the top-level panel carries a category; a child panel inherits
        # its parent's tab and Blender rejects a category on it.
