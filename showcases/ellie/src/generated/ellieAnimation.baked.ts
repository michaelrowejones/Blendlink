...
import { ellieAnimation } from './ellieAnimation.gen'
...
export function createBakedScene(
  ...
  const states = (ellieAnimation.states ?? {}) as Record<string, string | GroupedUrl>
  const defaultState = ellieAnimation.defaultState as string | null
  const bakeOutputs = (ellieAnimation.bakeOutputs ?? {}) as BakeOutputs
  const stateScales = (ellieAnimation.stateScales ?? {}) as StateScales
  const lightGroups = (ellieAnimation.lightGroups ?? {}) as Record<
    string,
    { url: string; maxValue: number } | GroupedLayer
  >
  const visibility = (ellieAnimation.stateVisibility ?? {}) as StateVisibility
  const textureVariants = ((ellieAnimation as typeof ellieAnimation & {
