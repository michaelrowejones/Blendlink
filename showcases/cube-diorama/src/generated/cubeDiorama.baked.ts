...
import { cubeDiorama } from './cubeDiorama.gen'
...
export function createBakedScene(
  ...
  const states = (cubeDiorama.states ?? {}) as Record<string, string | GroupedUrl>
  const defaultState = cubeDiorama.defaultState as string | null
  const bakeOutputs = (cubeDiorama.bakeOutputs ?? {}) as BakeOutputs
  const stateScales = (cubeDiorama.stateScales ?? {}) as StateScales
  const lightGroups = (cubeDiorama.lightGroups ?? {}) as Record<
    string,
    { url: string; maxValue: number } | GroupedLayer
  >
  const visibility = (cubeDiorama.stateVisibility ?? {}) as StateVisibility
  const textureVariants = ((cubeDiorama as typeof cubeDiorama & {
